...
class Main():
    ...
    def validar_datos(self, usuario, contrasenia) -> None:
        ...
        if usuario == "" or contrasenia == "":
            ...
            consulta = "SELECT * FROM USUARIOS WHERE NOMBRE_USUARIO=? AND CONTRASENIA=?"
            ...
            try:
                ...
                messagebox.showerror('BD', "El usuario no existe.")
                self.e_usuario.delete(0, END)
                self.e_contrasenia.delete(0, END)
            finally:
                cursor.close()
    ...
    def ventana_supervision(self) -> None:
        ...
        self.l_video.grid(row=0, column=0, columnspan=3)
    ...
    def subir_datos(self) -> None:
        ...
        if respuesta:
            datos = (
                ...
                self.descripcion, 
                self.id_usuario)
            consulta = "INSERT INTO CONTEOS VALUES(NULL, ?, ?, ?, ?, ?, ?, ?)"
